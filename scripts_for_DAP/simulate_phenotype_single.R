my_simulate<-function(file,num,causal_list,output,ld_file,weight_file)
 {
   #num=4
  ...
  # data<-data[,c(1:400)]
   ...
   if(!(is.na(weight_file)))
    {
      weight<-read.table(weight_file)
      my_weight<-weight[,2]
      pro=sapply(my_weight,function(x) 1/(1+exp(7.2-x*1.3)))
    ...
    #  causal<-array()
      for(i in 1:num)
       {
         site<-sample(3:dim(data)[2],1,replace=T,prob=pro)
         causal[i]<-site
       }
    } else{
      causal=runif(num,min=3,max=length(col))
      causal=as.integer(causal+0.5)
    }
    a<-vector()
    a<-causal
...
my_simulate(file_name,num,causal_list,out_file,ld_file,weight)
