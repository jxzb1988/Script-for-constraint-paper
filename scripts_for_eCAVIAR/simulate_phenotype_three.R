...
#   data=data[,-2]
 ...
 #   {
   ...
   phe=phe+data[,a[1]]*allel_eff[1]+data[,a[2]]*allel_eff[2]+data[,a[3]]*allel_eff[3]
   phe1=phe1+data[,a[1]]*allel_eff[1]+data[,a[2]]*allel_eff[2]+data[,a[3]]*allel_eff[3]
  #  }
 ...
 #  phe=phe+background
  ...
  # colnames(LD_r2_result)<-c("Causal","Effect_size","Allelic","Mean_LD")
   ...
   command=paste("/nv/hp10/bzeng30/data2/software/caviar-master/CAVIAR-C++/eCAVIAR", " -o ",output," -r 0.9 -l ", ld_file," -l "  ,ld_file, "-c 3  -z ",output1," -z ",output2)
   ...
   #command<-paste("perl combined_effect_size_and_CLPP.pl ",output_causal,causal_list,sep=" ")
